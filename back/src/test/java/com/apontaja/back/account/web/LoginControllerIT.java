...
class LoginControllerIT {
  ...
  private void createAccount(String email, String rawPassword) {
    accountRepository
        .save(new Account(idGenerator.generate(), email, passwordEncoder.encode(rawPassword), clock.instant()));
  }
  ...
  void login_reussi_renvoie_l_access_token_et_pose_le_cookie_refresh() throws Exception {
    ...
    mockMvc.perform(post("/api/auth/login").contentType(MediaType.APPLICATION_JSON).content("""
        ...
        .andExpect(jsonPath("$.email").value("ivy@example.com")).andExpect(cookie().exists("refresh_token"))
        .andExpect(cookie().httpOnly("refresh_token", true)).andExpect(cookie().path("refresh_token", "/api/auth"));
  }
