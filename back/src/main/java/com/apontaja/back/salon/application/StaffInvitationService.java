...
public class StaffInvitationService {
        ...
        StaffInvitationService(StaffInvitationRepository staffInvitationRepository,
                        ...
                        AccountQueryService accountQueryService, EmailSender emailSender,
                        OpaqueTokenGenerator opaqueTokenGenerator, TokenHasher tokenHasher, IdGenerator idGenerator,
                        Clock clock, @Value("${apontaja.security.staff-invitation.ttl:7d}") Duration invitationTtl,
        ...
        public CreateStaffInvitationResult createInvitation(CreateStaffInvitationCommand command) {
                ...
                accountQueryService.findAliveAccountIdByEmail(email)
                                .filter(accountId -> staffMembershipRepository
                                                .findAliveByAccountIdAndSalonId(accountId, command.salonId())
                                                .isPresent())
                                .ifPresent(accountId -> {
        ...
        public List<StaffInvitationSummary> listPending(UUID salonId) {
                return staffInvitationRepository.findPendingBySalonId(salonId).stream()
                                .map(invitation -> new StaffInvitationSummary(invitation.getId(), invitation.getEmail(),
                                                invitation.getRole().name(), invitation.getCreatedAt(),
        ...
        public StaffInvitationLookupResult lookup(String rawToken) {
                StaffInvitation invitation = staffInvitationRepository.findByTokenHash(tokenHasher.hash(rawToken))
                                .filter(inv -> inv.isUsable(clock.instant()))
                ...
                boolean accountExists = accountQueryService.findAliveAccountIdByEmail(invitation.getEmail())
                                .isPresent();

                return new StaffInvitationLookupResult(invitation.getId(), invitation.getEmail(),
                                invitation.getRole().name(), salonName, accountExists, invitation.getExpiresAt());
        }
        ...
        public void accept(UUID accountId, String rawToken) {
                ...
                if (staffMembershipRepository.findAliveByAccountIdAndSalonId(accountId, invitation.getSalonId())
                                .isPresent()) {
                        throw new AccountAlreadyStaffMemberException();
                }

                Instant now = clock.instant();
                staffMembershipRepository.save(new StaffMembership(idGenerator.generate(), accountId,
                                invitation.getSalonId(), invitation.getRole(), now));
